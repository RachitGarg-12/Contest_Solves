...
/*--------------------------------------------------------------------------------------------------------------------------*/
       ...
       bool poss(ll mid,map<ll,ll> fre,ll n,ll diff){
             ll rem=0,don=0;
             ll left= n-diff;
             for(auto i:fre){
                if(i.second>mid){rem += i.second-mid;}
                else{
                    ll more=(mid-i.second);
                    don += more/2;
                }
             }
             don +=(mid/2)*left;
        ...
        int main() {
            ...
            ll t;cin>>t;
            while(t--){
                ll n,m;cin>>n>>m;map<ll,ll> fre;
                ll a[m];fo(i,m){cin>>a[i];fre[a[i]]++;}
                ll diff=fre.size();
                ll l=1,r=m*2,ans=r;
                while(l<=r){
                     ll m=l+(r-l)/2;
                     if(poss(m,fre,n,diff)){ans=m;r=m-1;}
                     else{l=m+1;}
                }
             cout<<ans<<nl;
            }  
