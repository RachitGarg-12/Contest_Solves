    ...
    /*--------------------------------------------------------------------------------------------------------------------------*/
            ...
            int main() {
                ...
                ll tt;cin>>tt;
                while(tt--){       
                    ll n;cin>>n;set<string> s;string st[n];
                    fo(i,n){string h;cin>>h;st[i]=h;s.insert(h);}string ans="";
                    for(auto i:st){
                        ll g=i.size();bool b=false;
                      for(ll j=1;j<=g-1;j++){
                        string s1=i.substr(0,j);
                        string s2=i.substr(j,g-1);
                        // debug(j);debug(s1);debug(s2);
                        if(s.find(s1) !=s.end() && s.find(s2) != s.end()){ans+='1';b=true;break;} 
                      }
                     if(b==false){ans +='0';}  
                    }
                    cout<<ans<<nl;
