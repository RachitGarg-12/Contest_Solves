            ...
            int main() {
                ...
                ll tt;cin>>tt;
                 while(tt--){
                    ...
                    bool b=true;
                    if(n>=2*x){cout<<"YES"<<nl;continue;}
                    vector<ll> arr(n);arr=a;
                    sort(all(arr));
                    //diff of index needs to be >=x , so we can swap first and last n-x elements
                    // so ele from index [n-x+1,x] needs to be in sasme pos as sorted arr
                
                   fab(n-x,x){if(a[i]!=arr[i]){b=false;break;}}
