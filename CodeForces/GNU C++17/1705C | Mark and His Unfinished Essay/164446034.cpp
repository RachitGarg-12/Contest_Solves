    ...
    /*--------------------------------------------------------------------------------------------------------------------------*/
            ...
            int main() {
                ...
                while(tt--){       
                    int n,c,q;cin>>n>>c>>q;
                    string s;cin>>s;
                    vector<pair<ll,ll>> copy(c+1);vector<ll> len(c+1);len[0]=n;copy[0]={1,n};
                    f1(i,c){
                         ll l,r;cin>>l>>r;
                         copy[i]={l,r};len[i]=len[i-1]+r-l+1;
                    }
                    while(q--){
                        ll k;cin>>k;
                        while(k>n){
                        ll pos=lower_bound(all(len),k)-len.begin();
                        ll left=len[pos]-k;
                        k = copy[pos].second-left;
                        }
                       cout<<s[k-1]<<nl;
                    }
