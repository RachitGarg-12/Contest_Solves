...
pair<ll,ll> powg2(ll n){
    ...
    int main() {
       ...
       /* #ifndef ONLINE_JUDGE    
        ...
        ll t;cin>>t;
        while(t--){
           ll n,m;cin>>n>>m;ll a[n][m];ll ans=0;
           fo(i,n){fo(j,m){cin>>a[i][j];}}
           fo(i,n){ 
               fo(j,m){
                   ll s=a[i][j];
                   ll f=1,d=1;
                   while(i-f>=0 && j-f>=0){s += a[i-f][j-f];f++;}
                   f=1;
                   while(i-f>=0 && j+f<m){s += a[i-f][j+f];f++;}
                   f=1;
                   while(i+f<n && j+f<m){s += a[i+f][j+f];f++;}
                   f=1;
                   while(i+f<n && j-f>=0){s += a[i+f][j-f];f++;}
                   ans = max(ans,s);
               }
           }
            cout<<ans<<endl;
